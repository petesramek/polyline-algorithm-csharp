...
#pragma warning restore CA1508 // Avoid dead conditional code
    ...
    /// <summary>
    /// Tests that CopyTo with index throws ArgumentNullException when destination is null.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_NullDestination_ThrowsArgumentNullException() {
        // Arrange
        var polyline = Polyline.FromString("test");

        // Act & Assert
        _ = Assert.Throws<ArgumentNullException>(() => polyline.CopyTo(null!, 0));
    }

    /// <summary>
    /// Tests that CopyTo with index throws ArgumentOutOfRangeException when destinationIndex is negative.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_NegativeIndex_ThrowsArgumentOutOfRangeException() {
        // Arrange
        var polyline = Polyline.FromString("test");
        char[] destination = new char[10];

        // Act & Assert
        _ = Assert.Throws<ArgumentOutOfRangeException>(() => polyline.CopyTo(destination, -1));
    }

    /// <summary>
    /// Tests that CopyTo with index throws ArgumentOutOfRangeException when destinationIndex is greater than destination length.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_IndexGreaterThanLength_ThrowsArgumentOutOfRangeException() {
        // Arrange
        var polyline = Polyline.FromString("test");
        char[] destination = new char[10];

        // Act & Assert
        _ = Assert.Throws<ArgumentOutOfRangeException>(() => polyline.CopyTo(destination, 11));
    }

    /// <summary>
    /// Tests that CopyTo with index throws ArgumentException when destination has insufficient space from index.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_InsufficientSpaceFromIndex_ThrowsArgumentException() {
        // Arrange
        var polyline = Polyline.FromString("test");
        char[] destination = new char[6];

        // Act & Assert
        _ = Assert.Throws<ArgumentException>(() => polyline.CopyTo(destination, 3));
    }

    /// <summary>
    /// Tests that CopyTo with index copies characters starting at specified index.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_ValidIndexZero_CopiesCharactersFromStart() {
        // Arrange
        var polyline = Polyline.FromString("test");
        char[] destination = new char[4];

        // Act
        polyline.CopyTo(destination, 0);
    ...
    /// <summary>
    /// Tests that CopyTo with index copies characters starting at non-zero index.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_NonZeroIndex_CopiesCharactersFromIndex() {
        // Arrange
        var polyline = Polyline.FromString("abc");
        char[] destination = new char[10];

        // Act
        polyline.CopyTo(destination, 3);

        // Assert
        Assert.AreEqual('\0', destination[0]);
        Assert.AreEqual('\0', destination[1]);
        Assert.AreEqual('\0', destination[2]);
        Assert.AreEqual('a', destination[3]);
        Assert.AreEqual('b', destination[4]);
        Assert.AreEqual('c', destination[5]);
        Assert.AreEqual('\0', destination[6]);
    }

    /// <summary>
    /// Tests that CopyTo with index works with exact space from index.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_ExactSpaceFromIndex_CopiesCharacters() {
        // Arrange
        var polyline = Polyline.FromString("test");
        ...
        Assert.AreEqual('\0', destination[1]);
        Assert.AreEqual('\0', destination[2]);
        Assert.AreEqual('t', destination[3]);
        Assert.AreEqual('e', destination[4]);
        Assert.AreEqual('s', destination[5]);
        Assert.AreEqual('t', destination[6]);
    }

    /// <summary>
    /// Tests that CopyTo with index works correctly with empty polyline.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_EmptyPolyline_DoesNotThrow() {
        // Arrange
        var polyline = new Polyline();
        char[] destination = new char[5];

        // Act
        polyline.CopyTo(destination, 2);
    ...
    /// <summary>
    /// Tests that CopyTo with index throws ArgumentNullException for empty polyline with null destination.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_EmptyPolylineNullDestination_ThrowsArgumentNullException() {
        // Arrange
        var polyline = new Polyline();
    ...
    /// <summary>
    /// Tests that CopyTo with index throws ArgumentOutOfRangeException for empty polyline with negative index.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_EmptyPolylineNegativeIndex_ThrowsArgumentOutOfRangeException() {
        // Arrange
        var polyline = new Polyline();
        char[] destination = new char[5];
    ...
    /// <summary>
    /// Tests that CopyTo with index allows index equal to destination length for empty polyline.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_EmptyPolylineIndexEqualToLength_DoesNotThrow() {
        // Arrange
        var polyline = new Polyline();
        char[] destination = new char[5];

        // Act
        polyline.CopyTo(destination, 5);
    ...
    /// <summary>
    /// Tests that CopyTo with index allows index equal to destination length for non-empty polyline.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_IndexEqualToLengthNonEmptyPolyline_ThrowsArgumentException() {
        // Arrange
        var polyline = Polyline.FromString("test");
        char[] destination = new char[4];

        // Act & Assert
        _ = Assert.Throws<ArgumentException>(() => polyline.CopyTo(destination, 4));
    }

    /// <summary>
    /// Tests that CopyTo with index copies single character polyline.
    /// </summary>
    [TestMethod]
    public void CopyToWithIndex_SingleCharacter_CopiesCharacter() {
        // Arrange
        var polyline = Polyline.FromString("x");
        char[] destination = new char[5];
